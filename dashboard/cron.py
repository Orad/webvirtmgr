...
class MyCronJob(CronJobBase):
    ...
    def do(self):
        now = datetime.utcnow()
        month_first_day = now.replace(day=1,minute=0, hour=0, second=0, microsecond=0)
        today_start_date = now.replace(minute=0, hour=0, second=0, microsecond=0)
                ...
                for instance in instances:
                    ...
                    if running_histories:
                        for running_history in running_histories:
                            ...
                            if now.date() == running_history.created_at.date():
                                ...
                                if running_history.event == 'VIR_DOMAIN_PAUSED' and is_first_record == True:
                                    if compute.created_at.replace(tzinfo=None) < today_start_date: 
                                        today_time += Decimal((running_history.created_at.replace(tzinfo=None) - today_start_date).total_seconds() / 60)
                                    else:
                                        today_time += Decimal((running_history.created_at - compute.created_at).total_seconds() / 60)
                        ...
                        if first_running_history.event == 'VIR_DOMAIN_PAUSED':
                            if compute.created_at.replace(tzinfo=None) < month_first_day:
                                total_time += Decimal((first_running_history.created_at.replace(tzinfo=None) - month_first_day).total_seconds() / 60.0)
                            else:
                                total_time += Decimal((first_running_history.created_at - compute.created_at).total_seconds() / 60.0)
                        ...
                        if running_histories:
                            last_running_history = running_histories[len(running_histories)-1]
                            if last_running_history.event == 'VIR_DOMAIN_RUNNING':
                                total_time += Decimal(( now - month_first_day).total_seconds() / 60)
                                today_time += Decimal((now - today_start_date).total_seconds() / 60)
                    try:
